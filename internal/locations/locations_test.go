...
func TestCountIsSix(t *testing.T) {
        if got := Count(); got != 6 {
...
// A location that was in the old table but is no longer in the curated
// list must not resolve. This guards against stale references in
// generated configs.
func TestRemovedLocationsDoNotResolve(t *testing.T) {
        for _, code := range []string{"IT", "ES", "CA", "SG", "RU", "CN", "GB", "JP"} {
