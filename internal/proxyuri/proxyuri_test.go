...
// The headline promise: one user turns into exactly 50 configs.
func TestExpandProducesFiftyConfigs(t *testing.T) {
	p := ResolveParams(testUser(), testSettings(), VLESS)
	got := Expand(p)
	if len(got) != 50 {
		t.Fatalf("Expand() produced %d configs, want 50", len(got))
	}
...
// All 50 share one identity; only path and label vary. This is what keeps the
...
func TestCleanIPAppliesToAllConfigsAndKeepsHost(t *testing.T) {
	...
	cfgs := Expand(ResolveParams(u, testSettings(), VLESS))
	if len(cfgs) != 50 {
		t.Fatalf("got %d configs, want 50", len(cfgs))
	}
	for _, c := range cfgs {
